...
function getOAuthClient(settings: ReturnType<SettingsManager['load']>): OAuth2Client {
  const { clientId, clientSecret } = settings.providers.google_drive;
  const appUrl = settings.appUrl;
  if (!clientId || !clientSecret || !appUrl) {
    throw new Error('Google Drive is not configured. Visit Admin > Parachute to set it up.');
  }
  return new google.auth.OAuth2(clientId, clientSecret, `${appUrl}/parachute/oauth/google/callback`);
}

function encryptTokens(tokens: DriveTokens, secret: string): string {
  const key = crypto.scryptSync(secret, 'parachute-oauth', 32);
...
function decryptTokens(tokenRef: string, secret: string): DriveTokens {
  const key = crypto.scryptSync(secret, 'parachute-oauth', 32);
...
function createStateToken(userId: number | string, secret: string): string {
  const payload = JSON.stringify({ userId, timestamp: Date.now() });
  const signature = crypto.createHmac('sha256', secret).update(payload).digest('hex');
  return Buffer.from(JSON.stringify({ payload, signature })).toString('base64');
}

function verifyStateToken(state: string, secret: string): number | null {
  try {
...
export function handleOAuthConnect(req: Request, res: Response, logger: Logger, settings: ReturnType<SettingsManager['load']>): void {
  try {
    const client = getOAuthClient(settings);
    const state = createStateToken(req.session.user!.id, settings.cookieSecret);
...
export async function handleOAuthCallback(req: Request, res: Response, prisma: PrismaClient, logger: Logger, settingsMgr: SettingsManager): Promise<void> {
  try {
    const settings = settingsMgr.load();
    ...
    const userId = verifyStateToken(state, settings.cookieSecret);
    ...
    const tokenRef = encryptTokens(tokens as DriveTokens, settings.cookieSecret);
...
export async function getAuthStatus(userId: number | string, prisma: PrismaClient, logger: Logger, settingsMgr: SettingsManager): Promise<AuthStatus> {
  try {
...
export async function getAuthenticatedClient(userId: number | string, prisma: PrismaClient, logger: Logger, settingsMgr: SettingsManager): Promise<OAuth2Client | null> {
  try {
    const settings = settingsMgr.load();
    const rows: unknown[] = await prisma.$queryRaw`SELECT tokenRef FROM Parachute_Auth WHERE userId = ${userId}`;
    if (!rows.length) return null;
    const tokens = decryptTokens((rows[0] as { tokenRef: string }).tokenRef, settings.cookieSecret);
    const client = getOAuthClient(settings);
    client.setCredentials(tokens);
    return client;
  } catch (err) {
